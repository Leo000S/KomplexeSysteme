...
\title{Physics of Complex Systems: Exercise 8}
...
\section*{Task 8.2: Cross Entropy}
    ...
    \subsection*{(a)}
    Now, we do not consider $N$ individual experiments (like the fireflies), so the sum $ \sum_{i=1}^N$ is neglected. In 8.1, we had two summands for the two possible outcomes light and no light. In this task, this is replaced by the sum
    $\sum_{\mu=1}^M$ over all $M$ possible results.
    \begin{align}
    ...
    \Rightarrow q_\mu&=\frac{p_\mu}{\lambda} \label{qmu}
    \intertext{now sum both sides over all possible outcomes}
    \sum_{\mu=1}^{M} q_\mu = \frac{1}{\lambda} \sum_{\mu=1}^{M} p_\mu
    \intertext{now we apply the constraint $\sum_{\mu=1}^{M} q_\mu = 1$:}
    1= \frac{1}{\lambda} \sum_{\mu=1}^{M} p_\mu
    ...
    \end{align}
    Substituting $\lambda = 1$ back into the expression \eqref{qmu} for $q_\mu$:
    \begin{align}
        q_\mu = p_\mu \quad \forall \mu \in \{1 \dots M\}
    \end{align}
    With this, we know that the minimum coincides with the fact, that the calculated probability distribution ${q_\mu}$ is equal to the actual distribution ${p_\mu}$.
    To show that this is a global minimum, we calculate the second derivative
    \begin{align}
        \frac{\partial^2 \mathcal{L}}{\partial q_\mu^2} = \frac{p_\mu}{q_\mu^2} \gtrsim 0
    \end{align}
    The resulting components are not negative for all events $\mu$, so we obtain again a global minimum.

\end{document}

# lualatex -shell-escape Uebung8/exercise6/exercise8_Schaller_Lender.tex
